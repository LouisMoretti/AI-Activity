...
<Section title="Token activity">
  ...
  <div class="area">
    <div class="scroller" bind:this={scroller}>
      <!-- The calendar always sets the box, hidden under the other views, so
           switching tabs never changes its height, width or scroll. -->
      <div class="stack">
        <div class="calendar" class:hidden={view !== "daily"}>
    ...
    <div class="footer">
        ...
        <!-- Hidden, not removed, off Daily: the footer keeps its height. -->
        <div class="legend" class:hidden={view !== "daily"} aria-hidden="true">
...
<style>
  ...
  /* Narrow screens: the readout takes its own row, two lines tall (the
     longest one wraps), and the legend always the next one. */
  @media (max-width: 720px) {
    .readout { flex-basis: 100%; min-height: 2lh; }
  }
</style>
