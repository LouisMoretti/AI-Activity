<script lang="ts" generics="T extends string">
  let { options, value, onchange, label, variant = "pill", stretch = false }: {
    options: { value: T; label: string }[];
    value: T;
    onchange: (v: T) => void;
    label: string;
    variant?: "pill" | "tabs";
    /** Fill the row, options sharing it equally. */
    stretch?: boolean;
  } = $props();
</script>

<div class={variant} class:stretch role="group" aria-label={label}>
